...
const ScanningProgress: React.FC<ScanningProgressProps> = ({ progress, onCancel }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-16 p-8 bg-white dark:bg-slate-800 rounded-lg shadow-lg text-center">
      ...
      <div role="progressbar" aria-label={progress.status} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(percentage)} className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-4 mb-2">
        <div
