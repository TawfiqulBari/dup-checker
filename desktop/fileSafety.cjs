...
async function validateRegisteredFile(files, id) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || await fs.realpath(entry.absolute) !== entry.absolute || stat.size !== entry.size || stat.mtimeMs !== entry.mtime || (entry.ctime !== undefined && stat.ctimeMs !== entry.ctime) || stat.ino !== entry.ino) {
